...
exports.register = async (req, res) => {
  try {
    ...
    if (error.code === 11000) {
      ...
      return sendError(res, 400, 'Duplicate Error', [`${field} '${value}' is already registered.`]);
    }
...
exports.login = async (req, res) => {
  try {
    ...
    return sendSuccess(res, 200, 'User Login Successfully', { token });
  } catch (error) {
    console.log(error);
...
exports.changePassowrd = async (req, res) => {
  try {
    ...
    if (!user) {
      return sendError(res, 404, 'User not found or account has been deleted.');
    }
    ...
    if (oldPassword === newPassword) {
      return sendError(res, 400, 'Both old password and new password are required.');
    }
    ...
    if (ValidationError && validationError.errors.password) {
      return sendError(res, 400, 'Validation Error', [validationError.errors.password.message]);
    }
